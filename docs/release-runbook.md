...
- CI, native Linux isolation, `govulncheck`, schema validation, and SBOM
...
2. The release workflow builds natively on Linux amd64 and macOS arm64 because
   Tree-sitter requires CGO and therefore is not cross-compiled.
3. The built executable produces its own module-accurate, digest-bound SPDX
   2.3 SBOM. Each archive then receives an SBOM attestation through GitHub
   OIDC, followed immediately by `gh attestation verify` against the
   repository identity.
4. Each archive includes the project license and third-party notices.
5. The publish job creates SHA-256 checksums, attests them, and creates the
...
The release must not proceed if the live OSV proof, any native platform
isolation job, or the post-attestation verification is missing or skipped.
