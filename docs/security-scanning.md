...
Use `--dependency-reputation evidence.json` to add trusted, offline abandoned
...
Taint tracking is deterministic, syntax-aware, intraprocedural, and deliberately
bounded. It propagates multi-step assignment aliases and recognizes explicit
sanitizer calls. Complex containers, dynamic dispatch, implicit flows, and
interprocedural flows are not fully modeled. These limits create both false
...
rule source. All retain time, rule-count, byte, output, and temporary-file
limits. None is reported completed unless it actually ran.
