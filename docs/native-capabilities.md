...
| Malware analysis | native built-in signature evaluator plus trusted-source external YARA adapter |
| Semantic intent | deterministic local cross-file intent analysis plus opt-in model-backed review through a hardened provider boundary |
| Contract verification | declared-vs-observed capabilities and concrete allowlists |
| Behavioral assurance | deterministic tests; mediated artifact/private-workspace, structured-command, and public-HTTPS tools; native isolation on Linux, macOS, and supported Windows AppContainer systems |
...
| Integrated assurance | `skil assure` binds scan, verification, behavioral containment, native isolation, and host gateway enforcement |
