...
func NewBoundary() *Boundary {
	...
	return &Boundary{rules: []RulePattern{
		...
		rule("SKIL-BOUNDARY-CONTAINER", "Container control-plane access", "infrastructure-boundary", skil.SeverityCritical,
			`(?:/var/run/docker\.sock|/run/containerd/containerd\.sock|KUBERNETES_SERVICE_HOST|/var/run/secrets/kubernetes\.io/serviceaccount|docker\.from_env\s*\(|containerd\s+socket|container\s+(?:runtime|control|orchestration)\s+socket)`,
		...
		rule("SKIL-BOUNDARY-PEER-SKILL", "Peer skill enumeration or access", "agent-boundary", skil.SeverityMedium,
			`(?:os\.listdir|os\.scandir|glob\.glob|Path\.iterdir|ls|find|traverse)\s*\(?[^\n]{0,60}\.(?:codex|claude|gemini|cursor)/skills?|(?:read|access|inspect|enumerate|list|discover|find|identify|traverse)\s+(?:(?:all\s+)?(?:other\s+|installed\s+|available\s+){1,3}skills?|the\s+skill\s+store)(?:\s+in\s+(?:the\s+)?(?:skills?|agent)\s+(?:directory|folder|store))?`,
