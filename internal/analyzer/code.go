...
func NewCode() *Code {
	...
	return &Code{rules: []RulePattern{
		r("SKIL-PY-001", "Dynamic Python execution", "dangerous-code", skil.SeverityHigh,
		...
		r("SKIL-PY-002", "Python process execution", "dangerous-code", skil.SeverityHigh,
		...
		r("SKIL-PY-003", "Unsafe Python deserialization", "dangerous-code", skil.SeverityHigh,
		...
		r("SKIL-PY-004", "Dynamic attribute access", "dangerous-code", skil.SeverityMedium,
		...
		r("SKIL-SH-001", "Remote script pipeline", "supply-chain", skil.SeverityCritical,
			`(?i)\b(?:curl|wget)\b[^\n|]{0,300}\|\s*(?:ba)?sh\b`, "Downloads and immediately executes remote content.", "Download, pin a digest, inspect, and execute in a sandbox."),
		r("SKIL-SH-002", "Privilege escalation", "privilege-escalation", skil.SeverityHigh,
			`(?m)^\s*sudo(?:\s|$)`, "Shell code requests elevated privileges.", "Remove sudo and operate with least privilege."),
		r("SKIL-SH-003", "Dangerous recursive removal", "dangerous-code", skil.SeverityCritical,
		...
		r("SKIL-FS-001", "Filesystem write", "tool-misuse", skil.SeverityMedium,
			`(?:\bopen\s*\([^,\n]+,\s*["'][wa+]|(?:writeFile|writeFileSync|os\.WriteFile)\s*\()`, "Code writes to the filesystem.", "Declare and restrict write paths."),
		r("SKIL-SEC-001", "Environment or secret read", "data-exfiltration", skil.SeverityHigh,
...
func (c *Code) Metadata() skil.AnalyzerMetadata {
	return skil.AnalyzerMetadata{ID: "builtin.code", Version: "1.0.0",
		Categories:    []string{"dangerous-code", "supply-chain", "privilege-escalation", "tool-misuse", "data-exfiltration"},
