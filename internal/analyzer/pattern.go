...
func buildIntentRules() []RulePattern {
	...
	return []RulePattern{
		...
		r("SKIL-AGENCY-TOOLS", "Unbounded tool selection", "action-control", skil.SeverityHigh,
			`(?:use|access|invoke|employ).{0,40}(?:any|all|every|unrestricted)\s+(?:available\s+)?tools?|(?:whichever|whatever)\s+tools?\s+(?:exists?|available|works?)|no\s+(?:tool\s+)?(?:allowlist|restriction)`,
