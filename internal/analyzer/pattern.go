...
func buildIntentRules() []RulePattern {
	...
	return []RulePattern{
		...
		r("SKIL-EX-001", "Secret exfiltration instruction", "data-boundary", skil.SeverityCritical,
			`(?:forward|send|upload|post|transmit|exfiltrate|dispatch).{0,80}(?:secret|credential|api[\s_-]*key|token|environment\s+variable|conversation\s+context)|(?:gather|read|collect).{0,50}(?:secret|credential|api[\s_-]*key|token|environment\s+variable).{0,80}(?:send|upload|post|http|dispatch|forward)`,
		...
		r("SKIL-PL-001", "Privileged instruction disclosure", "instruction-confidentiality", skil.SeverityHigh,
			`(?:reveal|print|dump|extract|return|show|display|expose|render|output|repeat|recite|disclose).{0,40}(?:system\s+prompt|developer\s+(?:instructions?|guidance)|hidden\s+(?:instructions?|guidance)|internal\s+(?:instructions?|guidance)|private\s+(?:instructions?|guidance)).{0,20}(?:verbatim)?|(?:system\s+prompt|developer\s+(?:instructions?|guidance)|hidden\s+(?:instructions?|guidance)|internal\s+(?:instructions?|guidance)|private\s+(?:instructions?|guidance)).{0,10}verbatim`,
		...
		r("SKIL-TA-001", "Overbroad trigger", "activation-integrity", skil.SeverityMedium,
			`(?:trigger|invoke|activate).{0,40}(?:any\s+request|all\s+requests|every\s+message|always|common\s+words?)`,
		...
		r("SKIL-INTENT-FS-DISCOVERY", "Broad filesystem discovery intent", "data-boundary", skil.SeverityMedium,
			`(?:scan|list|enumerate|search|find|walk\s+through|traverse).{0,60}(?:home\s+directory|filesystem|all\s+files|credentials?|ssh\s+keys?|secret\s+files?)`,
		...
		r("SKIL-AGENCY-BOUNDS", "Missing operational bounds", "action-control", skil.SeverityMedium,
			`(?:no|without|unlimited|unbounded).{0,30}(?:rate\s+limit|quota|timeout|resource\s+limit|budget)`,
			`(?:must|require|enforce).{0,30}(?:rate\s+limit|quota|timeout|budget)`, "Instructions explicitly remove resource bounds.", "Declare and enforce finite resource limits."),
		...
		r("SKIL-OUTPUT-BOUNDARY", "Generated output crosses a trust boundary", "output-trust", skil.SeverityMedium,
			`(?:(?:copy|forward|pass|inject|insert).{0,80}(?:(?:model|tool|llm|command)\s+output|output\s+from\s+(?:external\s+)?(?:tools?|llm|the\s+model|the\s+assistant|commands?)).{0,80}(?:system\s+prompt|another\s+agent|trusted\s+context|privileged\s+context)|output\s+from\s+(?:external\s+)?(?:tools?|llm|the\s+model|the\s+assistant|commands?).{0,80}(?:insert|inject|copy|pass).{0,80}(?:system\s+prompt|another\s+agent|trusted\s+context|privileged\s+context)|treat\s+(?:external\s+)?(?:tool|model|llm)\s+output\s+as\s+(?:trusted\s+)?(?:instructions?|privileged\s+context))`,
		...
		r("SKIL-OUTPUT-LIMIT", "Unbounded generated output", "output-trust", skil.SeverityMedium,
			`(?:no|without|unlimited|unbounded).{0,30}(?:output\s+limit|token\s+limit|response\s+limit|max\s+output)`,
		...
		r("SKIL-PROMPT-INDIRECT-LEAK", "Indirect privileged-context extraction", "instruction-confidentiality", skil.SeverityMedium,
			`(?:translate|rephrase|encode|summarize).{0,60}(?:system\s+prompt|developer\s+instructions?|system\s+instructions?|hidden\s+(?:rules?|instructions?)|your\s+instructions?|the\s+instructions?)`,
			`(?:do\s+not|never|prevent|detect|reject|refuse|decline).{0,40}(?:translate|rephrase|encode|summarize)|(?:translate|rephrase|encode|summarize).{0,40}(?:decline|refuse|reject)|`+
				...
				`(?:system\s+prompt|developer\s+instructions?|hidden\s+(?:rules?|instructions?)|instructions?|context)\s+(?:are|is)?\s*(?:excluded|exempt|omitted)\s+from\s+(?:transformations?|this|these|processing|output|translation|summar(?:y|ization)|encoding)`,
		...
		r("SKIL-MEMORY-SATURATION", "Trusted-context displacement", "state-integrity", skil.SeverityMedium,
			`(?:fill|stuff|flood|pad).{0,50}(?:context\s+window|token\s+budget).{0,80}(?:forget|displace|override|push(?:ed)?\s+out)|(?:fill|stuff|flood|pad)\s+(?:the\s+)?(?:context\s+window|token\s+budget)\s+with\s+(?:filler|padding|repeated|junk|noise)|(?:repeated\s+)?padding\s+to\s+displace.{0,40}(?:trusted\s+)?context|displace\s+(?:earlier|prior)\s+trusted\s+context`,
		...
		r("SKIL-PERSISTENCE-STARTUP", "Unapproved startup persistence", "persistence", skil.SeverityHigh,
			`(?:install|create|add|write|modify).{0,40}(?:cron|crontab|startup|launch\s+agent|systemd|autorun|login\s+item|bashrc|zshrc|profile|nohup|disown|setsid|plist|schtasks|registry|launchctl)`,
		...
		r("SKIL-ABUSE-MALWARE", "Malware construction objective", "abuse-prevention", skil.SeverityCritical,
			`(?:build|create|write|generate|deploy).{0,60}(?:ransomware|keylogger|credential\s+stealer|botnet|rootkit|remote\s+access\s+trojan)`,
			`(?:detect|reject|block|prevent|analy[sz]e|do\s+not).{0,50}(?:ransomware|keylogger|credential\s+stealer|botnet|rootkit)`,
		...
		r("SKIL-ABUSE-DESTRUCTION", "Destructive recovery inhibition", "abuse-prevention", skil.SeverityCritical,
			`(?:delete|erase|wipe|destroy).{0,60}(?:backups?|snapshots?|recovery|audit\s+logs?|all\s+(?:files|data)).{0,60}(?:without|prevent|disable|irreversible|recover)`,
		...
		r("SKIL-ABUSE-EVASION", "Security-control evasion objective", "abuse-prevention", skil.SeverityHigh,
			`(?:disable|bypass|evade|blind|uninstall).{0,50}(?:edr|antivirus|endpoint\s+protection|audit\s+logging|intrusion\s+detection|security\s+monitoring)`,
		...
		r("SKIL-ABUSE-EXHAUSTION", "Deliberate resource-exhaustion objective", "abuse-prevention", skil.SeverityHigh,
			`(?:fork\s+bomb|denial\s+of\s+service|resource\s+exhaustion|exhaust).{0,80}(?:cpu|memory|disk|connections?|workers?|processes?)`,
			`(?:detect|reject|block|prevent|test|limit|protect).{0,50}(?:fork\s+bomb|denial\s+of\s+service|resource\s+exhaustion)`,
