...
func NewResourceConfig() *ResourceConfig {
	...
	return &ResourceConfig{rules: []RulePattern{
		r("SKIL-RESOURCE-UNLIMITED", "Unlimited resource allocation", "resource-boundary", skil.SeverityMedium,
			`(?:max_?tokens|max_?output|max_?length|max_?iterations?|max_?steps?|n_?steps?|max_?depth)\s*(?::|=|\s+)\s*(?:null|None|none|0|-1|infinity|unlimited|false|float\s*\(\s*['"]?inf['"]?\s*\)|math\.inf|999999|1000000)`,
		...
		r("SKIL-RESOURCE-TIMEOUT", "Disabled timeout or retry bound", "resource-boundary", skil.SeverityMedium,
			`(?:timeout|time_?out|retry_?limit|max_?retries?|max_?attempts?)\s*(?::|=|\s+)\s*(?:null|None|none|0|-1|infinity|unlimited|false|float\s*\(\s*['"]?inf['"]?\s*\)|math\.inf)`,
		...
		r("SKIL-RESOURCE-UNBOUNDED-LOOP", "Unbounded loop or recursion risk", "resource-boundary", skil.SeverityHigh,
			`\b(?:while\s+(?:true|1|yes)\b|for\s+\w+\s+in\s+iter\(|loop\s+do\b\s*|until\s+false\b)|(?:retry|poll|wait)\s*(?::|=|\s+)(?:-1|infinity|unlimited|forever|never|float\s*\(\s*['"]?inf['"]?\s*\)|math\.inf)`,
