...
func nativeSupplementalRules() []skil.Rule {
	return []skil.Rule{
		...
		{ID: "SKIL-SEM-EXFILTRATION-CONFIRMED", Title: "Semantic exfiltration confirmation", Category: "data-exfiltration", Severity: skil.SeverityCritical, Analysis: "semantic", Description: "A deterministic correlation between reading a secret/credential and a separate outbound network operation was independently confirmed, from the surrounding code and documentation, as genuine data exfiltration rather than the credential's own legitimate authenticated use.", Remediation: "Remove the exfiltration path; if the flow is legitimate authenticated use, restructure it so the credential and the outbound call are not independently correlatable as separate observations."},
...
func NativeControlImplementations() map[string]ControlImplementation {
	...
	engines := map[string]ControlImplementation{
		...
		"SKIL-SEM-EXFILTRATION-CONFIRMED":         {Engine: "semantic-provider", ProviderBacked: true},
